...
cdef extern from 'misc.h' nogil:
    cdef:
        ...
        void fill_idw_wts_arr(DT_D *x_arr,
                              DT_D *pow_arr,
                              DT_D idw_exp,
                              DT_L len_x)
        void fill_mult_arr(DT_D *x,
                           DT_D *y,
                           DT_D *mult_arr,
                           DT_L len_x)
...
cpdef DT_D get_idw(DT_D idw_x,
    ...
    cdef:
        ...
        np.ndarray[DT_D, ndim=1] idw_wts = \
            np.zeros(shape=len_x, dtype=np.float64)
        np.ndarray[DT_D, ndim=1] dists = \
            np.zeros(shape=len_x, dtype=np.float64)
        np.ndarray[DT_D, ndim=1] mult_arr = \
            np.zeros(shape=len_x, dtype=np.float64)
...
cpdef np.ndarray get_idw_arr(DT_D[:] idw_x_arr,
                             DT_D[:] idw_y_arr,
                             DT_D[:] xs,
                             DT_D[:] ys,
                             DT_D[:] zs,
    ...
    cdef:
        ...
        np.ndarray[DT_D, ndim=1] idw_z_arr = \
            np.zeros(shape=len_idw_z, dtype=np.float64)


    for i in range(len_idw_z):
        idw_z_arr[i] = get_idw(idw_x_arr[i],
                               idw_y_arr[i],
                               xs,
                               ys,
                               zs,
...
cpdef void slct_nebrs_cy(const DT_D x,
                         const DT_D y,
    ...
    cdef:
        ...
        DT_D min_dist, curr_min_ang, curr_max_ang, x_dist, y_dist, nebr_ang
        DT_D quad_ang_incr
    ...
    for i in range(n_nebs):
        dists_arr[i] = get_dist(x, y, nebrs_x_crds_arr[i], nebrs_y_crds_arr[i])
        idxs_fin_arr[i] = 0
    ...
    for i in range(n_quads):
        ...
        for j in range(n_nebs):
            ...
            if (nebr_ang >= curr_min_ang) and (nebr_ang < curr_max_ang):
                ...
                slctd_nebrs_dists_arr[j] = get_dist(x, y, nebrs_x_crds_arr[j], nebrs_y_crds_arr[j])

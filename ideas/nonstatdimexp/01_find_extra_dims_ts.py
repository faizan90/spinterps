...
from cyth import get_evgs_tvgs_diff_01
...
def get_l2_norm(crds_1, crds_2):
    ...
    return (((crds_1 - crds_2) ** 2).sum(axis=1)) ** 0.5
...
#       dtype='object')
    ...
    out_dir = Path(f'test_temp_1961_1961_ts_{stn}_01')
...
#     time_fmt = '%Y-%m-%d'
    ...
    ts_vals = pd.read_csv(
        in_data_file, sep=sep, index_col=0).loc[beg_time:end_time, stn].values
    ...
    lags, evgs, _ = get_lags_evgs_tvgs(ts_vals, crds, n_lags, f'600 {fit_vg_str}(200000)')
...
#         polish=False)
    ...
    lags, evgs, tvgs = get_lags_evgs_tvgs(ts_vals, crds, n_lags, fit_vg_str)

...
def main():
    ...
    assert np.isfinite(crds_df.loc[:, ['X', 'Y', 'Z']].values).all()
    ...
    for ref_col in ref_cols:
        ...
        ref_x, ref_y, ref_z = crds_df.loc[ref_col, ['X', 'Y', 'Z']]
        ...
        vg_cloud = get_vg_cloud3(
            tss_df, ref_col, ref_x, ref_y, ref_z, crds_df, ignr_cols, use_axis)
...
def get_vg_cloud3(
    ...
    for col in tss_df.columns:
        ...
        vg_val_b = (ref_z - crds_df.loc[col, 'Z']) ** 2

        if vg_val_b <= 1e-2: continue
        ...
        vg_val_a = np.mean(vg_tss)

        vg_val = vg_val_a * vg_val_b

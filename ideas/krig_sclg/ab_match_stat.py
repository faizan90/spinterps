...
def get_slr_mlt(ref_men, ref_std, dst_std, tst_etn_vie_grd, tst_grd):
    ...
    lft_std = np.nanstd(get_sld_grd(
        tst_grd, dtn_lsr, dtn_gtr, slr_lft, True, False, ref_men))

    rht_std = np.nanstd(get_sld_grd(
        tst_grd, dtn_lsr, dtn_gtr, slr_rht, True, False, ref_men))
    ...
    while tol > min_tol:
        ...
        mde_std = np.nanstd(get_sld_grd(
            tst_grd, dtn_lsr, dtn_gtr, slr_mde, True, False, ref_men))
    ...
    if fal_flg:
        ...
        tst_grd_fnl = get_sld_grd(
            tst_grd, dtn_lsr, dtn_gtr, slr_fnl, True, False, ref_men)
...
def get_sld_grd(grd, lsr, gtr, slr, non_neg_flg, ref_men_flg, ref_men):
    ...
    if ref_men_flg: sld_grd *= ref_men / np.nanmean(sld_grd)

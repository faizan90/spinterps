...
def main():
    ...
    raise NotImplementedError(
        'Use ac_match_stat.py. It is slower but more reliable!')
    ...
    # save_stats_sers(dta_dfe, ncf_pth, stt_pth, plt_pth, interp_labels)
...
def get_slr_mlt(
    ...
    opt_ags = (
        tst_grd, dtn_lsr, dtn_gtr, non_neg_flg, men_ccn_flg, ref_men, dst_std)
    ...
    opt_res = minimize(
        ...
        opt_ags,
        bounds=np.array(
            [[0.0, max(ref_std / dst_std, dst_std / ref_std) * 2.0]]))

    fal_flg = False  # not opt_res.success
...
def obj_ftn(pms, grd, lsr, gtr, non_neg_flg, men_ccn_flg, ref_men, dst_std):

    slr, = pms

    sld_grd = get_sld_grd(
        grd, lsr, gtr, slr, non_neg_flg, men_ccn_flg, ref_men)

    grd_std = np.nanstd(sld_grd)
    return (grd_std - dst_std) ** 2
